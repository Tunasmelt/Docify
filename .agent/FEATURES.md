...
**Tests:**
- `apps/api/tests/test_embedder.py` — 23 tests originally, **26 as of 2026-07-27**: fake-client unit tests (fast, no network) including chunk<->vector correspondence and cardinality-mismatch tests, real-`voyageai.Client`-with-patched-transport tests for retry/error behavior (exercises the SDK's actual tenacity retry logic), one test against real chunker output on `table_heavy.pdf`, one test confirming Voyage's real tokenizer is available without an API call, 3 new lazy-construction tests (2026-07-27 follow-up), and one real-API test gated behind `RUN_REAL_VOYAGE_TEST=1` (skipped by default)
**Acceptance criteria:**
...
- [x] **Added 2026-07-27 (lazy-construction fix, closes a gap flagged during FEAT-009's rerank follow-up):** `Embedder()` no longer builds a real `voyageai.Client()` at construction — deferred to first actual `embed()`/`embed_query()` call, same discipline FEAT-017's audit established for `GeminiOcrClient`/`OcrSpaceClient`. Before this fix, a bare `Embedder()` (and by extension `Retriever()`, which builds a default `Embedder()`) crashed immediately with `voyageai.error.AuthenticationError` in any environment missing `VOYAGE_API_KEY`, even for callers that never call `embed()`/`embed_query()` at all. Also fixed a related inconsistency found while closing this: `embed()`'s tokenizer-resolution step (`_get_tokenizer()`) touches the client *before* `embed()`'s own try/except, so a missing key there previously would have raised a raw `AuthenticationError` instead of `EmbedError` — `embed()`'s own docstring promises `EmbedError` for exactly this class of failure ("auth, invalid request, ..."). Fixed by catching `VoyageError` inside `_get_tokenizer()` itself. `test_embedder_construction_never_touches_network_or_requires_api_key`, `test_embedder_embed_fails_with_embed_error_not_a_raw_sdk_crash_when_api_key_absent`, `test_embedder_embed_query_fails_with_embed_error_not_a_raw_sdk_crash_when_api_key_absent`; `test_retriever.py` gained `test_bare_retriever_construction_never_requires_voyage_api_key` confirming the fix closes the gap transitively for `Retriever()` too. **Fresh-clone check re-run** (same methodology that originally caught the OCR-tier version of this bug): real `git clone` + diff-apply, `uv sync` from lockfile alone (no code changes needed — no new dependency), `uv run pytest` with no `.env` present — 45 passed, 1 skipped (gated real-API test), 0 crashes, confirming the fix holds in a genuinely fresh environment, not just under the existing dev `.venv` which already has `VOYAGE_API_KEY` set locally.
...
**Changelog:** See CHANGELOG.md 2026-07-23 "feature: Voyage embedder wrapper (FEAT-006)" and 2026-07-27 "fix: Embedder/Retriever eager-construction crash on missing VOYAGE_API_KEY"
...
### [FEAT-019] Conversation memory in prompt
**Phase:** 2
**Status:** complete
**Owner:** claude-code
**Depends on:** FEAT-010 (Generator), FEAT-012 (`/query`), FEAT-026 (`list_messages_for_conversation`)
**Files:**
- `apps/api/services/generator.py` — `Generator.generate()` gained an optional `history: list[dict] | None = None` param; new `_build_history_parts()`, `_strip_citation_markers()`, `_HISTORY_HEADER`; `SYSTEM_INSTRUCTION` extended with an explicit history-is-context-not-citable instruction
- `apps/api/routes/query.py` — new `MAX_HISTORY_TURNS = 5` constant; fetches prior messages via `queries.list_messages_for_conversation(..., limit=MAX_HISTORY_TURNS * 2)` when `conversation_id` is provided, passes them to `generator.generate(..., history=prior_messages)`
- `apps/api/db/queries.py` — `list_messages_for_conversation()` gained an optional `limit: int | None = None` param (default `None` = unchanged behavior for `GET /conversations/{id}/messages`, its only other caller)
**Tests:**
- `apps/api/tests/test_generator.py` — 4 new tests: no-history is a no-op (byte-identical prompt to before this feature existed), history folds in before the numbered chunks, historical `[N]` markers are stripped from folded-in assistant answers, and — the key regression guard for item 4 — a new answer's citation numbering is unaffected by marker-shaped text sitting in the history block
- `apps/api/tests/test_query.py` — 6 new tests: first turn gets empty history, second turn receives the first turn's real Q&A as history, the bounded window truncates correctly (more turns than `MAX_HISTORY_TURNS` produces exactly the most recent ones), a same-turn user/assistant ordering regression test (see the real bug below), history-fetch isolation for the new `limit=` code path (a different real user gets nothing), and one real, gated end-to-end quality+latency test
**Design decision — minimal memory, no query rewriting:** prior Q&A pairs fold into `Generator`'s prompt so a follow-up like "how does that compare to X?" resolves correctly in the ANSWER. Retrieval itself is unchanged — `Retriever.retrieve()` still searches using only the raw current-turn `question`; no query reformulation/rewriting was built in this pass. Matches this project's established "ship minimal, measure before adding speculative complexity" pattern from reranking's rollout (`.agent/MEMORY.md`'s 2026-07-27 decision). **Known limitation, stated explicitly, not an oversight:** a follow-up whose wording alone wouldn't retrieve well on its own — one that relies entirely on unstated context from a prior turn, with no lexical/semantic overlap with the actual target content — may retrieve poorly even though `Generator` has the conversational context to answer well once the right chunks are found. Query rewriting (folding conversation context into the retrieval query itself, not just the generation prompt) would close this gap but was explicitly out of scope for this pass.
**Bounded window:** `MAX_HISTORY_TURNS = 5` (10 messages: 5 user + 5 assistant) — a real, stated choice, not an arbitrary round number. Ordinary follow-up ambiguity ("that", "it", "how does that compare") resolves against the immediately preceding turn or two in practice; 5 gives generous margin beyond that without letting a long-running conversation's prompt size grow unbounded. A token-budget scheme was considered and rejected: it would need its own tokenizer call just to enforce and isn't obviously better-justified at this project's scale than a fixed turn count.
**Citation numbering across turns, confirmed not disturbed (task item 4):** citation numbering resets every turn by construction — `chunks` passed to `generate()` is always THIS call's own list, so `[N]` always means `chunks[N-1]` of only the current turn, matching FEAT-026's existing per-message marker persistence (each stored message's `content` keeps its own turn's numbering, never renumbered later). Defended two ways, not just asserted: (1) a prior turn's `[N]` markers are stripped entirely from the folded-in history text before it ever reaches the prompt (`_strip_citation_markers`), so they can't be mistaken for anything meaningful; (2) `_parse_citations()` only ever scans the model's own freshly-generated `response.text`, never the prompt it was given, so even marker-shaped text sitting in history (tested directly with `[1]`, `[2]`, `[9]`, `[99]` planted in fake history content) cannot leak into the current turn's `cited_indices`/`hallucinated_markers`.

**A real bug found and fixed while building the bounded fetch (task item 3):** the first implementation of `list_messages_for_conversation(..., limit=N)` fetched `ORDER BY created_at DESC LIMIT N` at the DB level and reversed the result in Python to restore oldest-first order, reasoning that this would avoid a full-conversation scan for a long-running conversation. This is wrong for a case that happens on *every single turn*: a turn's user and assistant messages are inserted inside the **same** `create_query_turn()` transaction, and Postgres's `now()` (the `created_at` column's default) is transaction-start time, not per-statement time — both rows get an **identical** `created_at`. A `DESC` query does not reliably invert the `ASC` order for tied rows (observed live returning user-before-assistant even under `DESC`), so reversing it silently swapped a turn's two messages. Caught by a real test (`test_conversation_history_with_limit_preserves_user_then_assistant_order_within_a_turn`) before this ever reached a real conversation. Fixed by reusing the exact same already-proven `ORDER BY created_at ASC` query unconditionally and slicing the last `N` rows in Python — the accepted tradeoff is fetching a conversation's full history (still a cheap, `messages_conv_idx`-backed index scan on `conversation_id`, not a table scan) rather than only the tail, judged fine at this project's portfolio scale rather than assumed free.

**Multi-tenant isolation, re-confirmed at this new surface (task item 7):** rather than assuming the new `limit=` branch inherits FEAT-026's isolation proof untested, `test_conversation_history_fetch_with_limit_is_scoped_to_the_requesting_user` calls `list_messages_for_conversation(..., limit=10)` directly with a different real user's id against a conversation they don't own — confirmed to return `[]` — with a positive control (the real owner gets their real history back) proving the empty result is real scoping, not a broken query. This is on top of, not instead of, the pre-existing `get_conversation()` ownership check that already 404s a foreign `conversation_id` before history is ever fetched.

**Real two-turn conversation against real ingested content (task item 5), `table_heavy.pdf`, real Docling+Voyage+Gemini throughout, no dependency overrides:**
| Turn | Question | Real answer | latency_ms |
|---|---|---|---|
| 1 | What is Angola's Human Development Index value in 2010? | "According to Table 19, Angola's Human Development Index (HDI) value in 2010 is 4.42 [1]." | 3762 |
| 2 | How does that compare to the year 2000? | "In the year 2000, Angola's Human Development Index (HDI) value was 4.42 [4]. Compared to the 2010 value of 4.42, Angola's HDI value remained unchanged [4]." | 4674 |

Turn 2's wording ("that", "the year 2000") only makes sense with turn 1's context — the real answer shows it correctly resolved "that" to "Angola's 2010 HDI value" and correctly used Table 19's real data (the Angola row is genuinely 4.42 in both the 2000 and 2010 columns) to answer accurately, not generically or confusedly.

**Real latency measurement (task item 6), isolating history's own effect from the confound of turn 1 vs turn 2 asking different questions:** same question ("How does that compare to the year 2000?"), same retrieved chunks, `Generator.generate()` called directly with `history=None` vs `history=<turn 1's real fetched history>`, 3 trials with ABBA counterbalancing (which condition goes first alternates each trial, so position-in-sequence and any latency drift across the ~2-minute run are spread evenly across both conditions):

| Condition | mean | min | max |
|---|---|---|---|
| without history | 7213.7ms | 4763.2ms | 8593.3ms |
| with history | 3606.9ms | 3283.9ms | 3799.4ms |

**Surprising, real, reproduced result: adding history made generation FASTER (mean −3606.9ms), not slower.** A single fixed-order pair was tried first and showed the same direction even more starkly (2.9-3.0s vs 9.5-10.7s means across two attempts) — initially suspected as a call-order/warm-connection artifact (history was always the second of two back-to-back calls), so the measurement was redone with ABBA counterbalancing specifically to rule that out. The gap persisted just as strongly, ruling out simple call-order as the explanation. Most likely real mechanism (not fully instrumented here to confirm via output token counts, flagged rather than overclaimed): with history, the model knows exactly what "that"/"the year 2000" refers to and can answer directly and concisely; without it, given only an ambiguous follow-up against a multi-country/multi-year table, the model most likely has to work harder to infer or hedge about the intended referent, plausibly producing a longer answer — and for autoregressive generation, output token count (not prompt/input size) is the dominant real latency driver, not input size. **No update to the ~4-8.3s end-to-end latency budget (`.agent/reviews/2026-07-24-full-flow.md`) is warranted** — the real measured effect of conversation memory on generation latency was neutral-to-positive in this test, not an increase, so there is nothing to add to the budget on the evidence gathered.

**Run:**
- `pytest apps/api/tests/test_generator.py apps/api/tests/test_query.py -v` (fast — the real quality test below is skipped by default)
- `RUN_CONVERSATION_MEMORY_QUALITY_TEST=1 pytest apps/api/tests/test_query.py::test_real_two_turn_conversation_uses_first_turns_context_to_answer_the_second -v -s` (slow, real Docling+Voyage+Gemini calls, ~4 min)

**Changelog:** See CHANGELOG.md 2026-07-27 "feature: conversation memory in Generator's prompt (FEAT-019)"

---

### [FEAT-016] SSE streaming responses
**Phase:** 2 (moved out of Phase 4 — see Decision below)
**Status:** tested
**Owner:** claude-code
**Depends on:** FEAT-012 (`/query`), FEAT-011 (Verifier), FEAT-026 (citation figure URLs), FEAT-019 (conversation memory), the chat UI (`claude-design`-owned, Phase 3)
**Files:**
- `apps/api/services/generator.py` — new `Generator.generate_stream()` (async generator; yields `str` text deltas via `client.aio.models.generate_content_stream`, then one final `GenerateStreamResult`), new `GenerateStreamResult` dataclass
- `apps/api/routes/query.py` — new `POST /query/stream` route + `_stream_query_events()`; existing `post_query()` refactored (behavior-preserving — same 55 existing tests pass unchanged) to share `_validate_payload()`/`_check_ownership()`/`_load_history()` with the new route
- `apps/api/tests/test_query_stream.py` (new)
- `apps/web/lib/api/query.ts` — new `askQuestionStream()` (`fetch()` + manual `ReadableStream` read; browsers' `EventSource` can't send a POST body or `Authorization` header)
- `apps/web/components/chat/loading-stages.tsx` — rebuilt from a 3-stage `setInterval`-driven fake into a 2-state (`retrieving`/`verifying`) component driven by real SSE events; the old middle "READING…" stage is gone — real streamed text fills that gap now
- `apps/web/app/(app)/chat/[conversation_id]/page.tsx` — `ask()` rewritten around `askQuestionStream()`'s handlers; `loadingStage: number | null` replaced with `streamingId: string | null` + `streamingStage: "retrieving" | "verifying" | null`
**Tests:**
- `apps/api/tests/test_query_stream.py` — full event sequence + supported/partial/unsupported citation handling (mirrors `test_query.py`'s equivalent tests exactly, against the streaming path), mid-stream generation failure, post-generation verification failure
- Real end-to-end: real Docling+Voyage+Gemini through the actual `/query/stream` HTTP endpoint, a real browser (Playwright, real sign-in against real local Supabase Auth) driving the actual chat UI

**Design decisions, stated explicitly before implementing (per this feature's own task brief):**
1. **What streams — Gemini generation tokens, progressively**, not just stage-transition events. Confirmed live: `genai.Client` exposes `client.aio.models.generate_content_stream()` (an async coroutine returning an async iterator, confirmed against the installed SDK source, not assumed) on the *same* client instance `Generator` already builds. This directly addresses the 3.4–9.3s wait FEAT-012 measured; a stage-only stream would still leave the user staring at nothing for most of it.
2. **Citation timing** — claim-span extraction needs the complete answer's real sentence boundaries, so verification cannot start before generation finishes; this is a hard dependency, not a choice. Raw streamed text (including un-stripped `[N]` brackets) renders as plain inert text — never styled or clickable — until `citations-resolved` arrives with the final, marker-stripped answer and real verdicts. The `token`→`citations-resolved` gap is communicated via a `verifying` (`"CHECKING SOURCES…"`) indicator client-side, matching `loading-stages.tsx`'s existing visual language.
3. **Endpoint shape — new route `POST /query/stream`**, not a mode flag on `/query`. `response_model=QueryResponse` and `StreamingResponse` are mutually exclusive in FastAPI; a separate route keeps `/query`'s existing synchronous contract (and everything depending on it — `test_query_e2e.py`, any future non-browser caller) completely untouched.

**Event sequence** (real, fixed, confirmed via both protocol-level tests and real client-side event-arrival logging — see the real bug below): `retrieving -> token* -> verifying -> citations-resolved -> done`, with `error` able to replace any step from `token` onward and always terminating the stream. Full contract in `API_CONTRACT.md`.

**Auth-over-SSE, confirmed via a real test, not code review alone (task requirement):** `JWTAuthMiddleware.dispatch()` completes the full JWT/JWKS check and returns 401 *before* calling `call_next()` — structurally, an invalid/missing token means the route (and thus any streaming) never executes. Verified live: no-header, garbage-JWT, and (for validation/ownership) a non-owned `document_id` and an empty `document_ids` all return a plain `application/json` 401/403/422 body with zero `event:`-shaped bytes and zero SSE content-type — never a stream that opens and then rejects.

**Mid-stream failure, confirmed via real tests (task requirement, priority item 7's safety-adjacent twin):** a fake `Generator.generate_stream()` raising `GenerationError` partway through (after real `token` deltas already streamed) produces `retrieving, token, token, error` — no `verifying`/`citations-resolved`/`done` ever follows, `verify_batch()` is asserted never called. A fake `Verifier.verify_batch()` raising an unexpected exception (distinct from the documented Gemini-call-failure fail-safe, which `Verifier` already converts to a safe `UNSUPPORTED` `Verdict` rather than raising) after generation completes produces `retrieving, token, verifying, error` — no `citations-resolved`/`done`. Both cases return HTTP 200 (SSE: failure is *in* the stream, not the HTTP status) and the connection always closes on a terminal `error` or `done`, never a silent hang.

**Priority item — re-verified FEAT-011/012's citation-correctness rules against the streaming path specifically, not assumed inherited:** `test_query_stream.py` mirrors `test_query.py`'s exact assertions (supported kept as-is, partial kept with its marker never dropped, unsupported dropped from the response *and* still persisted for audit with its marker stripped from `citations-resolved.answer`) against `/query/stream` directly — same fakes, same real-ingested-chunk fixtures, same verdict-handling code path (the streaming route deliberately reuses `_extract_claim_spans()`/`_strip_dropped_markers()`/the citation-building loop verbatim, not a second implementation that could silently drift). All pass. No new path exists where an unverified claim could render as verified — `token` events carry only raw, un-styled text; only `citations-resolved` (which only ever fires after `verify_batch()` returns) can attach a verdict to a marker.

**A real bug found and fixed via genuine browser testing, not caught by protocol-level tests alone:** every synchronous/blocking call inside `_stream_query_events()` (`Retriever.retrieve`, `Verifier.verify_batch`, `fetch_generator_chunks`, `signed_figure_url`, `create_query_turn`) is wrapped in `asyncio.to_thread(...)`. Calling `verify_batch()` directly (no `await`, the initial implementation) froze the asyncio event loop for its entire real ~8s Gemini-verification duration — confirmed live that this prevented uvicorn from ever flushing the already-`yield`-ed `verifying` frame to the socket until the *next* yield, so the client received `verifying` and `citations-resolved` at the identical timestamp instead of with the real gap between them (a temporary client-side event-arrival console log, `performance.now()` per event, made this directly observable — DOM-sampling alone wasn't conclusive since it was ambiguous whether "no visible change" meant "no event" or "event arrived but didn't render"). Both `test_stream_e2e.py` (a raw httpx protocol test) and the FastAPI `TestClient`-based unit tests happened to mask this — their HTTP clients' own read buffering apparently coalesced the frames the same way, so only a real browser + real network stack exposed it. Fixed; re-verified live in a real browser (Playwright) showing a genuine multi-second `verifying` indicator window before resolution.

**Real time-to-first-token vs old time-to-complete, measured against the real endpoint (task requirement):** for a short factual answer (`table_heavy.pdf`, "What is Angola's HDI value in 2010?"), Gemini's own real streaming delivers only 3 chunks clustered in a ~25ms burst near the very end of generation — confirmed this is genuine Gemini behavior, not a bug in this stack, by calling `Generator.generate_stream()` directly with no HTTP/ASGI/middleware involved at all and observing the identical clustering. Time-to-first-token for this case (6.6s) is close to non-streaming's time-to-complete (8.4s) — a modest win. For a longer, open-ended answer on the same document, real progressive delivery is dramatic and unambiguous: 20 real `token` events spread across a genuine ~2.0s window, first token at 8.09s vs non-streaming's 25.45s time-to-complete — **17.35s earlier** visible content. **Honest, non-overclaimed finding: streaming's UX benefit is real but proportional to answer length** — large for longer/multi-fact answers (most of this app's real usage, per FEAT-012's own quality fixtures), modest for very short ones, because Gemini itself, not this implementation, chunks short completions coarsely near the end of its own generation.

**Real browser verification (task's "start the dev server, use the feature in a browser" standard):** signed in as a real local Supabase Auth user (no production values touched — `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY` explicitly overridden to local values at `next dev` startup, since `.env.local` points at the same live project as the backend's `.env`, matching this task's mandatory Step 0 environment-safety discipline), asked a real question against a real ingested `table_heavy.pdf`, confirmed: the `retrieving` skeleton renders before any text, real answer text grows character-by-character in the DOM (not a single jump), the `verifying` "CHECKING SOURCES…" indicator appears for the real verification window (post-fix), the final message swaps to the resolved answer with a clickable, correctly-verdict-colored `[N]` citation marker (`data-verdict="supported"`/`"partial"`) showing the right page number, and zero browser console errors throughout.

**Decision — FEAT number.** `FEAT-016` was already reserved for exactly this ("Streaming responses via SSE — planned", Phase 4 placeholder list) — confirmed by reading this file first, matching this project's established discipline, not assumed. Filed in Phase 2 (immediately after FEAT-019, its most recent dependency) rather than the original Phase 4 placement, since it's a direct extension of `/query`'s own pipeline, not a Phase 4 polish/deploy item — the placeholder list entry is removed below.

**Run:**
- `pytest apps/api/tests/test_query_stream.py -v`
- `curl -N -X POST -H "Authorization: Bearer <jwt>" -d '{"question":"...","document_ids":["..."]}' http://localhost:8000/query/stream`

**Changelog:** See CHANGELOG.md 2026-07-27 "feature: SSE streaming for /query (FEAT-016)"
...
(FEAT-016 moved to Phase 2 and built 2026-07-27 — see its entry above, filed after FEAT-019.)
