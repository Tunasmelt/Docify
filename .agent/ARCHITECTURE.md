...
│  └─ Voyage / Anthropic SDK       │   │  └─ Storage (files)  │
└─────┬────────────────────────────┘   └──────────────────────┘
      │
┌─────▼──────────────┐  ┌────────────────┐  ┌───────────────────┐
│  Voyage API        │  │  Anthropic API │  │  Gemini API       │
│  multimodal-3.5    │  │  Sonnet + Haiku│  │  Flash (OCR only) │
│  (embed + rerank)  │  │  (gen + verify)│  │                   │
└────────────────────┘  └────────────────┘  └───────────────────┘
```
...
| Generation | Claude | Sonnet (latest) | Strong citation-following behavior, $120 credits available |
| Verification | Claude | Haiku (latest) | Cheap LLM-as-judge for citation grounding |
...
4. FastAPI:
   ...
   f. Calls Claude Sonnet with prompt; response includes inline [N] citation markers
...
│   │   ├── generator.py          Claude Sonnet wrapper
│   │   └── verifier.py           Claude Haiku citation-check
│   ├── db/
...
- **Thin Next.js API routes** — Next handlers only do auth check + forward. All business logic lives in FastAPI. Frontend never talks to Voyage/Claude/Docling directly.
...
- [x] **Claude for gen + verify while credits last.** Post-Sep-19 provider is an open decision.
...
- [ ] Post-credit-expiry generation provider (Claude paid tier vs. Gemini swap)
