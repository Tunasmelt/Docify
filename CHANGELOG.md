...
**Rollback:** Run the `ROLLBACK` block at the bottom of `apps/api/migrations/migration1script.sql` against the live project via the dashboard SQL editor (drops policies, storage buckets, tables, then enum types, in dependency order). Revert `.agent/FEATURES.md` and `.agent/SCHEMA.md` to the prior commit.
